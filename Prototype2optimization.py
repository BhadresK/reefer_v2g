...
# ═══════════════════════════════════════════════════════════════════════════════
#  SECTION 10 – ADDITIONAL ANALYTICAL GRAPHS
# ═══════════════════════════════════════════════════════════════════════════════
...
def plot_additional_analysis(
    ...
    fig.suptitle(
        "V2G Additional Analysis - Financial / Grid / Operational\n"
        f"(SoC 20-100% | E_max = {v2g.E_max:.0f} kWh | TRU = 0 kW | Binary Mutex MILP)",
    ...
    ax2.bar(hours, spread * 1000, width=0.22,
            color=np.where(spread > v2g.deg_cost_eur_kwh, "#43A047", "#E53935"),
            alpha=0.8, label="V2G spread (EUR/MWh)")
    ax2.axhline(v2g.deg_cost_eur_kwh * 1000, color="black", ls="--",
                lw=1.5, label=f"deg cost = {v2g.deg_cost_eur_kwh*1000:.0f} EUR/MWh")
    ax2.axhline(0, color="grey", lw=0.8)
    plug_mask = A.plugged > 0.5
    for i in range(len(hours) - 1):
        if plug_mask[i]:
            ax2.axvspan(hours[i], hours[i] + 0.25, alpha=0.06, color="blue")
    ax2.set_title("2 Arbitrage Spread  (V2G price - buy price)",
                  fontsize=10, fontweight="bold")
    ax2.set_xlabel("Hour of day"); ax2.set_ylabel("Spread  (EUR/MWh)")
    ...
    for r, lbl, col in zip(results_list, labels_short, colors_list):
        total_kwh = np.sum(r.p_charge + r.p_discharge) * dt
        if total_kwh > 0:
            deg_per_slot = (r.p_charge + r.p_discharge) / total_kwh * r.deg_cost_eur_day
        else:
            deg_per_slot = np.zeros_like(r.p_charge)
        cum = np.cumsum(
            (r.p_charge * r.price_buy - r.p_discharge * r.price_v2g + deg_per_slot) * dt
        )
        ax3.plot(hours, cum, color=col, lw=2, label=lbl)
    ax3.axhline(0, color="grey", lw=0.8)
    ax3.fill_between(hours, 0,
                     np.cumsum((A.p_charge * A.price_buy) * dt),
                     alpha=0.08, color=COL["dumb"])
    ax3.set_title("3 Cumulative Net Cost Over Day  (running EUR)",
                  fontsize=10, fontweight="bold")
    ax3.set_xlabel("Hour of day"); ax3.set_ylabel("Cumulative cost  (EUR)")
    ...
    im = ax4.imshow(power_matrix, aspect="auto", cmap="RdYlGn",
                    vmin=-v2g.p_d_max, vmax=v2g.p_c_max,
                    extent=[0, 24, -0.5, 3.5])
    plt.colorbar(im, ax=ax4,
                 label="Net power  (kW)  [green=charge, red=discharge]",
                 shrink=0.85)
    ax4.set_yticks([0, 1, 2, 3])
    ax4.set_yticklabels(labels_short[::-1])
    ax4.set_title("4 Net Power Heatmap  (P_c - P_d  per scenario/hour)",
                  fontsize=10, fontweight="bold")
    ...
    legend_patches.append(mpatches.Patch(
        facecolor="grey", hatch="///", label="V2G revenue (hatched, reduces bar)"
    ))
    ax5.legend(handles=legend_patches, fontsize=7, loc="upper right")
    ax5.set_title("5 Cost/Revenue Breakdown by Season & Scenario",
                  fontsize=10, fontweight="bold")
    ax5.set_ylabel("EUR/day  (charge cost + deg - V2G revenue)")
    ...
    ax6.set_title("6 Grid Power Flow  (P_c - P_d)",
                  fontsize=10, fontweight="bold")
    ...
    ax7.set_title("7 SoC Risk Corridor  (SoC - E_min headroom)",
                  fontsize=10, fontweight="bold")
    ...
    def annual(r_key: str) -> float:
        return (
            all_season_results["winter"][r_key].cost_eur_day * 130 +
            all_season_results["summer"][r_key].cost_eur_day * 131 +
            all_season_results.get(
                "winter_weekend", {}
            ).get(r_key, all_season_results["winter"][r_key]).cost_eur_day * 52 +
            all_season_results.get(
                "summer_weekend", {}
            ).get(r_key, all_season_results["summer"][r_key]).cost_eur_day * 52
        )

    ann_costs = [annual(k) for k in ["A", "B", "C", "D"]]
    base = ann_costs[0]
    bars = ax8.bar(["A-Dumb", "B-Smart", "C-MILP", "D-MPC"],
                   ann_costs, color=colors_list, alpha=0.85, edgecolor="black")
    for bar, v in zip(bars, ann_costs):
        saving = base - v
        ax8.text(bar.get_x() + bar.get_width() / 2, v + 15,
                 f"EUR{v:,.0f}\n({saving:+,.0f} vs A)",
                 ha="center", va="bottom", fontsize=7.5, fontweight="bold")
    ax8.set_title("8 Annual Net Cost per Trailer  (all seasons combined)",
                  fontsize=10, fontweight="bold")
    ax8.set_ylabel("Annual net cost  (EUR/year)")
...
def print_report(
    ...
    for r in results.values():
        ...
        print(
            f"    {r.scenario:<32}  annual cost: EUR{r.cost_eur_day*365:>8,.0f}"
    ...
    print(
        f"\n  Degradation tipping point: V2G profitable up to ~EUR{tipping:.3f}/kWh  "
        f"({tipping_ok})"
    )
    print()
...
def main() -> None:
    ...
    print("  Binary mutex | SoC 20-100% | TRU=0 kW | No fallback")
    print("=" * 65)
    ...
    answer = input(
        "  Include battery degradation cost in optimisation? [Y/n]: "
    ).strip().lower()
    if answer in ("n", "no"):
        ...
        answer2 = input(
            f"  Keep default EUR{v2g.deg_cost_eur_kwh:.3f}/kWh? [Y / enter value]: "
        ).strip()
    ...
    print(
        f"  -> Arrival SoC: {soc_init_pct:.0f}%   "
        f"Departure SoC: 100% = {v2g.E_max:.0f} kWh (fixed)"
    )
    print("-" * 55)
    ...
    for season, dwell_type, days_per_year, label in DAY_TYPES:
        ...
        print(
            f"  Buy: EUR{buy.min()*1000:.1f}-{buy.max()*1000:.1f}/MWh  |  "
            f"V2G peak: {v2g_p.max()*1000:.1f} EUR/MWh  |  "
        ...
        plot_all(v2g, hours, A, B, C, D, deg_df, season=label, out=out_png)
